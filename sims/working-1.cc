...
int main(int argc, char *argv[]) {
    ...
    staPositionAlloc->SetAttribute("X", StringValue("ns3::UniformRandomVariable[Min=5.0|Max=15.0]"));
    ...
    // Schedule periodic disconnections for STA0
    Ptr<Ipv4> ipv4 = staNodes.Get(0)->GetObject<Ipv4>();
    int32_t ifIndex = ipv4->GetInterfaceForDevice(staDevices.Get(0));
    
    // Disconnection pattern: 5-8s, 12-15s, 20-23s
    for (double time = 5.0; time < simTime; time += 7.0) {
        Simulator::Schedule(Seconds(time), &Ipv4::SetDown, ipv4, ifIndex);
        Simulator::Schedule(Seconds(time + 3.0), &Ipv4::SetUp, ipv4, ifIndex);
    }
